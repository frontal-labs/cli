...
journey_auth() {
  run_journey "auth-status" "$CLI" auth whoami --local --json
}

journey_config() {
  run_journey "config-list" "$CLI" config list --json
}
...
journey_output() {
  local results
  results="["
  results+="$(run_journey "output-json" "$CLI" auth whoami --local --json)"
  results+=","
  results+="$(run_journey "output-yaml" "$CLI" auth whoami --local --yaml)"
  results+="]"
  echo "$results"
}
